...
class MainEventLoop {
    ...
    */
    def processOneEvent (l: List[Entity]) (ev: Event): (List[Entity], List[Event]) = {
        ...
        val (applies, doesNotApply) = partition(l)(ev)
        val (newEntities, newEvents) = applies.foldLeft((List.empty[Entity], List.empty[Event])) {
            case ((entitiesAcc, eventsAcc), e) if ev.appliesTo(e) =>
                val (ents, evs) = e.handleEvent(this)(ev)
                (entitiesAcc ++ ents, eventsAcc ++ evs)
            case ((entitiesAcc, eventsAcc), e) => (entitiesAcc :+ e, eventsAcc)
        }
        (doesNotApply ++ newEntities, newEvents)
   ...
   */
    final def performOneTransition: (List[Entity], List[Event]) = {
        ...
        events.foldLeft((entities, List.empty[Event])) { case ((currentEntities, currentEvents), event) =>
            val (newEntities, freshEvents) = processOneEvent(currentEntities)(event)
            (newEntities, freshEvents ++ currentEvents)
  }
    }
