...
case object BaloonEscaped extends Event {
    ...
    override def appliesTo(e: Entity): Boolean = e match {
        case agm: ArcadeGameManager => true
        case _ => false
    }
}
...
case class Boom(x: Double, y: Double) extends Event {
    override def appliesTo(e: Entity): Boolean = e match {
        case ArcadeBaloon(_, x1, y1, _) => {
           ...
           math.sqrt((x - x1) * (x - x1) + (y - y1) * (y - y1)) <= ScreenDimensions.baloonBurstDistance 
        }
        case _ => false
    }
}
...
case class ArcadeGameManager(val gamePoints: Int) extends Entity {
    ...
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) = e match {
        case Tick(_) => {
            ...
            if (rand < 0.02) {
                val newBaloon = ArcadeBaloon(global.numEntities + 1, 20, (500 + 100*Random.nextDouble() + 1), (4 + 8*Random.nextDouble() + 1))
                (List(this, newBaloon), Nil)
            } else if (rand > 0.98) {
                val newBaloon = ArcadeBaloon(global.numEntities + 1, ScreenDimensions.width - 20, (500 + 100*Random.nextDouble() + 1), (4 + 8*Random.nextDouble() + 1))
                (List(this, newBaloon), Nil)
            } else {
                (List(this), Nil)
            }
        }
        //TODO: Add points given by ScreenDimensions.pointsBaloonEscaped, no new events
        case BaloonEscaped => (List(this.copy(gamePoints = this.gamePoints + ScreenDimensions.pointsBaloonEscaped)), Nil) 
        //TODO: Add points given by ScreenDimensions.pointsBaloonBurst, no new events
        case BaloonBurst => (List(this.copy(gamePoints = this.gamePoints + ScreenDimensions.pointsBaloonBurst)), Nil) 
        // TODO: Empty list of entities and events to stop the trampoline.
        case Stop => (Nil, Nil) 
        //TODO: Return this entity unchanged and the empty list of events.
        case _ => (List(this), Nil) 
    }
...
case class Dart(id: Int, x: Double, y: Double) extends Entity{
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) = e match  {
        case Tick(delta) => {
           val y1 = y - delta * ScreenDimensions.dartSpeed
            if (y1 <= 20) {
                (List(), List())
            } else {
